...
do
  ...
  if [ "$HAS_PK" = "0" ]; then
    csql -udba -c "create table ${TBNAME} (a char(${RECSIZE})) dont_reuse_oid" $_DBNAME
  else
    csql -udba -c "create table ${TBNAME} (a char(${RECSIZE}) primary key) dont_reuse_oid" $_DBNAME
  fi
